<template>
  <section>
    ...
    <el-table :data="orders"
      ...
      <el-table-column label="自动评价倒计时" align="center" width="140">
        <template slot-scope="scope">
          <counter :endtime='scope.row.evaluateTime' v-if='scope.row.evaluateTime'></counter>
        </template>
      </el-table-column>
      <el-table-column label="操作" align="center" min-width="220">
        <template slot-scope="scope">
          <el-button type="info" size="small" @click="handelShow(scope.row.oId)"
                     v-show="rules.indexOf('01010301') >= 0">查看
          </el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script>
  ...
  const params = JSON.stringify({type: 3})
    ...
    methods: {
      _getMyAllUOrder () {
        ...
        getMyAllUOrder(params).then((res) => {
          if (res.success) {
            this._calculatePagination(res.result.data)
          } else {
      ...
      _calculatePagination (data) {
        this.totalData = data.reverse()
        data.map(function (item, index) {
          let time = +new Date(item.orderTime) + 3600000
